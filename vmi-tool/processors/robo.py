...
# Robo uses the same CSV format as Revolut Trading
INCLUDE = {
    "CASH TOP-UP": "II",
    "DIVIDEND": "IV",
}
...
def process(csv_content: str, year: int) -> list[dict]:
    ...
    required = {"Date", "Type", "Total Amount", "Currency"}
    if reader.fieldnames and not required.issubset(set(reader.fieldnames)):
        raise ValueError(
            "CSV neatitinka Robo Advisor formato. "
